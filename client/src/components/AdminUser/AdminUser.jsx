...
const AdminUser = () => {
    ...
    const dataTable = usersData?.length > 0 && usersData?.map((user) => {
        return {
            ...user, key: user.id,
            role: <Select style={{ width: '100%' }} onChange={(value) => handleOnchangeSelectRole(value, user.id)} defaultValue={user.role} options={optionRoles} />,
            action: <ButtonComponent textbutton={'Cập nhật'} onClick={() => handleUpdate(user.id)} />
        }
    })
    return (
        <div>
